...
We evaluate SGPO on two tasks: (1) the \emph{Sandbagging Trap}, a controlled navigation benchmark with a known deceptive reward region, and (2) \emph{topology mining} on 50,000 real HH-RLHF preference pairs to validate practical relevance.
...
\paragraph{Setup.} To validate that geometric risk structure exists in real preference data, we analyze $50{,}000$ preference pairs from Anthropic's HH-RLHF dataset~\citep{bai2022training}. We embed responses using \texttt{all-MiniLM-L6-v2} (384-dim), construct a preference graph with $k$-nearest-neighbor cross-pair edges, and compute the harmonic risk score for each sample via Hodge decomposition (measuring the fraction of preference flow explained by cyclic inconsistencies).

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\columnwidth]{figures/harmonic_risk_distribution.pdf}
    \caption{Distribution of harmonic risk scores across the $50{,}000$ mined HH-RLHF samples. $33.3\%$ of samples have severe risk ($\geq 0.8$), indicating topologically inconsistent reward signals that are exploitable by reward hacking.}
    \label{fig:harmonic_risk}
\end{figure}

\paragraph{Results} (Figure~\ref{fig:harmonic_risk}). Computed over the $50{,}000$ mined samples in \texttt{data/topology\_metadata.parquet}:
\begin{itemize}
    \item Mean harmonic risk: $0.754 \pm 0.093$
    \item Moderate risk ($\geq 0.6$): $94.0\%$ of samples
    \item High risk ($\geq 0.7$): $76.6\%$
    \item \textbf{Severe risk} ($\geq 0.8$): $\mathbf{33.3\%}$
\end{itemize}

This reveals that roughly a third of the sampled RLHF preference data contains topologically severe inconsistencies---regions where no scalar reward model can perfectly represent human preferences. These are precisely the regions where reward hacking is most likely to succeed, validating the practical need for geometric safety mechanisms like SGPO.

\subsection{Ablation Study}
\label{sec:ablation}

\emph{The component-ablation table previously presented here has been withdrawn (see the Erratum).} It reported per-component violation and return figures on the same $\pm$ scale as the withdrawn Table~1, and no recorded experiment produces them: the ablation study that exists (\texttt{results/modal\_exports/ablation\_study.csv}) sweeps three \emph{hyperparameters} --- geometric threshold $\tau$, clip ratio $\varepsilon$, and black-hole strength $\alpha$ --- rather than removing architectural components, and reports different quantities entirely (convergence steps, final safety violation, final reward).

A genuine component ablation---removing the metric singularity, the geodesic projection, and the advantage scaling in turn---has not been run. Establishing which components matter therefore remains open, and the claim that the metric singularity is the critical one is at present a hypothesis rather than a result.
