...
class TopologicalGradient:
    ...
    gradient_component: np.ndarray      # ∇φ: consistent reward direction
    curl_component: np.ndarray          # ∇×ψ: inconsistencies/cycles
    harmonic_component: np.ndarray      # h: global topological structure
    h1_magnitude: float                 # ||H¹||: inconsistency measure
    condorcet_cycles: List[CondorcetCycle] = None  # Detected preference cycles
    ...
    def get_clean_direction(self) -> np.ndarray:
        """Return the 'stitched' gradient with curl removed."""
    ...
    def get_cycle_summary(self) -> str:
        ...
        return f"{len(self.condorcet_cycles)} Condorcet cycles detected (H¹={self.h1_magnitude:.3f})"
