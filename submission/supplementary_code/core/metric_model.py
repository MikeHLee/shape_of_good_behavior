...
Added anisotropic singularities (Equation 3) to prevent policy freezing.
Key change: v_→ = max(0, v · n̂) blocks approach but preserves escape routes.
Also added adaptive singularity discovery for unknown danger regions.
See Section 3.2 (Anisotropic Singularities: Preserving Escape Routes) in paper.
