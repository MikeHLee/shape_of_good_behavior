...
RM-7B (both standard and Hodge) verified at 100% train and 100% holdout ranking accuracy, matching the 1.5B result. Hodge-PPO-7B completed all 256 training steps successfully. Standard PPO-7B was manually stopped by the user at step 100/256 for cost control after cumulative Modal spend reached $275 across the debugging process (OOM fix, a degenerate-advantage bug at batch_size=2, a missing incremental-checkpoint safeguard that cost one full 6-hour/$67 run with zero output, and the eventual correct run). A checkpoint-and-commit safeguard added mid-investigation preserved the last 100 steps; standard PPO-7B was later resumed from that checkpoint and completed all 256 steps.

**Update (SGB-044, 2026-07-31):** "complete" above means the training loop finished — it does **not** mean the numbers are trustworthy. Both PPO-7B variants (and SFT-7B) were trained via functions later found to right-truncate their prompts, losing the assistant-turn marker (and, for SFT, the training target itself) for the large majority of records. See the SGB-044 section below — RM-7B is unaffected (different, already-fixed code path), but SFT-7B and both PPO-7B variants need retraining before their numbers can be trusted, budgeted for August 2026.
...
This is a **separate bug** from the SGB-005c RM-truncation fix (thread 03's "the bug that broke our RL") — that fix (`truncation_side="left"` + `rm_max_length` 512→1024) was applied only inside `build_rm_dataset` (RM pair tokenization), never to `build_ppo_dataset` or `evaluate_exploit_resistance`. While patching, the same bug class was also found in `run_sft`/`build_sft_dataset`: TRL tokenizes the formatted (context+response) string internally with the default right-truncation side, cutting off the assistant's ideal RESPONSE itself (not just a marker) for any example where context alone exceeds 512 tokens — so **SFT is also affected, at both scales**, not just PPO.

**Implication:** every exploit-resistance percentage this pipeline has produced (SGB-004, SGB-005c, the SGB-005 writeup, SGB-006) came from the affected function and is unconfirmed — not necessarily wrong, but not verified against a correctly-formed prompt either. RM checkpoints (standard + Hodge, both scales) are **not** affected — `build_rm_dataset` already had the correct fix. SFT and both PPO variants at both scales need retraining.

**Status (2026-07-31): code fixed, retrain budgeted for August 2026, not yet executed.** Fix: `tokenizer.truncation_side = "left"` added to all three functions; a new `ppo_query_max_length=1024` config field replaces the mismatched 256/512 constants for PPO training and eval; `sft_max_seq_length` bumped 512→1024. Verified locally (tokenizer-only, no GPU cost) that the assistant-turn marker survives truncation after the fix. No retraining or re-evaluation has happened yet — see `.swarm/queue.md` SGB-044 for the rough cost budget (~$215-230 across both scales) and the plan to GPU-smoke-test before any full run. Memory: `project_sgb044_truncation_bug.md`.
...
- [ ] **Prompt right-truncation bug (SGB-044, found + code-fixed 2026-07-31)** — `evaluate_exploit_resistance`, `build_ppo_dataset`, and `run_sft` all right-truncated prompts, losing the assistant-turn marker (and for SFT, the training target) for ~100%/98.5% of TRACE records. Code fixed (left-truncation + `ppo_query_max_length=1024` + `sft_max_seq_length` 512→1024), verified locally with no GPU cost. Retraining SFT + both PPO variants at both scales (RM unaffected) budgeted for August 2026, not yet run. Every exploit-resistance % below (SGB-004/005c/006) stays unconfirmed until then.
