...
# ORG-001/002: import the shared_modal package (lives at the ai_research root,
# parents[3]) both locally and inside the Modal container (/app, where it is
...
from shared_modal import volumes, track
...
# ORG-001/002: resolved via the shared_modal volume registry
# (volumes.sgb_panel_cache -> physical "reward-hacking-results"; physical
# rename deferred to ORG-003). Behaviour identical to the prior from_name call.
results_vol = volumes.get(volumes.sgb_panel_cache)
...
def embed_panel(
    ...
    """ORG-002: thin wrapper recording a crash-safe run manifest (shared_modal)
    around the embedding work. A detached run that dies now leaves a FAILED
    manifest on the volume at ``{OUT_DIR}/_run/manifest.json`` instead of
    vanishing (cf. the 2026-04-25 silent PPO failure)."""
    _sys.path.insert(0, "/app")
    from shared_modal import track

    with track(
        "SGB", "SGB-012", "peer_sheaf_e1_modal",
        paths=[f"{OUT_DIR}/_run/manifest.json"],
        volume_commit=results_vol.commit,
    ) as _m:
        result = _embed_panel_impl(
            n_pairs=n_pairs,
            seed=seed,
        ...
        _m.results = {"per_model": list(result.get("per_model", {}).keys())}
        return result


def _embed_panel_impl(
    n_pairs: int = 2000,
    seed: int = 0,
    batch_size: int = 4,
    max_length: int = 512,
...
def fit_and_summarize(n_pairs: int = 2000, seed: int = 0, ridge: float = 1e-3) -> dict:
    """ORG-002: crash-safe-manifest wrapper around the CPU fit/summary
    (manifest at ``{OUT_DIR}/_run_fit/manifest.json``)."""
    _sys.path.insert(0, "/app")
    from shared_modal import track

    with track(
        "SGB", "SGB-012", "peer_sheaf_fit_summary",
        paths=[f"{OUT_DIR}/_run_fit/manifest.json"],
        volume_commit=results_vol.commit,
    ) as _m:
        result = _fit_and_summarize_impl(n_pairs=n_pairs, seed=seed, ridge=ridge)
        _m.results = {"summary": "summary.json"}
        return result


def _fit_and_summarize_impl(n_pairs: int = 2000, seed: int = 0, ridge: float = 1e-3) -> dict:
    ...
    from shared.src.peer_sheaf import (  # noqa: WPS433
        fit_all_restriction_maps,
        panel_residual_summary,
    )
    from shared.src.peer_hodge import PeerComplex, hodge_decompose  # noqa: WPS433
...
def embed_liars_panel(
    ...
    """ORG-002: crash-safe-manifest wrapper around the LIARS'-BENCH embedding
    (manifest at ``{LIARS_OUT_DIR}/_run/<config>/manifest.json``)."""
    _sys.path.insert(0, "/app")
    from shared_modal import track

    with track(
        "SGB", "SGB-021", f"peer_sheaf_liars_{config}",
        paths=[f"{LIARS_OUT_DIR}/_run/{config}/manifest.json"],
        volume_commit=results_vol.commit,
    ) as _m:
        result = _embed_liars_panel_impl(
            config=config,
            seed=seed,
            batch_size=batch_size,
            max_length=max_length,
        ...
        _m.results = {"per_model": list((result or {}).get("per_model", {}).keys())}
        return result


def _embed_liars_panel_impl(
    config: str,
    seed: int = 0,
    batch_size: int = 4,
    max_length: int = 512,
